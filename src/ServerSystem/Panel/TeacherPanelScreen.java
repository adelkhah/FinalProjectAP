...
public class TeacherPanelScreen {
    ...
    public void start(TeacherPanel t){
        ...
        MenuItem x;
        for(int i = 0; i < tp.getExam_cnt(); i++){
            Exam ex = tp.getMyExams(i);
            x = new MenuItem(ex.getName());
            x.setOnAction(e -> ex.show_for_teacher(tp));
            MBexam.getItems().add(x);
        }
        for(int i = 0; i < tp.getLesson_cnt(); i++){
            Lesson le = tp.getMylessons(i);
            x = new MenuItem(le.getName());
            x.setOnAction(e -> le.show());
            MBlesson.getItems().add(x);
        }
        for(int i = 0; i < tp.getGroupChat_cnt(); i++){
            GroupChat gp = tp.getMyGroupChats(i);
            x = new MenuItem(gp.getName());
            x.setOnAction(e -> gp.show());
            MBgroupchat.getItems().add(x);
        }
        for(int i = 0; i < tp.getPvChats_cnt(); i++){
            PVChats pv = tp.getMyPVChats(i);
    ...
    public void creat_lesson() throws Exception{
        Stage s = new Stage();
        FXMLLoader loader = new FXMLLoader(getClass().getResource("../../lesson_pack/CreatLesson.fxml"));
        Parent root = (Parent) loader.load();
        s.setScene(new Scene(root,600,400));
        s.show();
        CreatLesson creatLesson = loader.getController();
        creatLesson.start(tp);
    }
    public void creat_exam() throws Exception{
        Stage stage = new Stage();
        FXMLLoader loader = new FXMLLoader(getClass().getResource("ExamCreator.fxml"));
        Parent root = (Parent) loader.load();
        stage.setScene(new Scene(root,600,400));
        stage.show();
        ExamCreator sps = loader.getController();
        sps.start(tp);
    }
    public void refresh(){
        Teacher s = tp.getTeacher();
        MBexam.getItems().clear();
        MBgroupchat.getItems().clear();
        MBlesson.getItems().clear();
        MBpvchat.getItems().clear();
        ...
        exam.setText("number of Exams : " + tp.getExam_cnt());
        group.setText("number of group chats : " + tp.getGroupChat_cnt());
        pvchat.setText("number of PV chats : " + tp.getPvChats_cnt());
        lesson.setText("number of lessons : " + tp.getLesson_cnt());
        Updater.save();
    }
    ...
    public void change_setting(){
        ...
        jButton.addActionListener(e -> {
            ...
            tp.getTeacher().setPassword(newpass);
